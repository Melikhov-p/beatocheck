...
@dp.message_handler(chat_id=config['MODER_CHAT'], commands=['decline_reason'])
async def process_send_decline_reason(message: types.Message):
    ...
    #                                 f'{args[1]}')
    await bot.send_message(config['MODER_CHAT'], f'@{message.from_user.username}, эта команда в данный момент недоступна.')
